...
if [ "$GEOSERVER_URL" == "" ]; then
    env
    # configure GEOSERVER_URL if started using docker-compose or --link flag
    if [ -n "$GEOSERVER_PORT_8080_TCP_ADDR" ]; then
        GEOSERVER_URL="http://${GEOSERVER_PORT_8080_TCP_ADDR}:${GEOSERVER_PORT_8080_TCP_PORT}/"
    fi
    echo ${GEOSERVER_URL}
fi
